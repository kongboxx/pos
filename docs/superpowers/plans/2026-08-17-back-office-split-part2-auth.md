...
- [x] **Step 1: วัดตัวเลขจริงก่อนเขียน**

```bash
pnpm test 2>&1 | grep -E "Tests +[0-9]+ passed"
```

> **วัดจริง 2026-08-17:** shared 413 · api 407 · web 249 · office 120 · print-agent 15 · web-kit 14 = **1,218** (92 ไฟล์เทสต์)
...
- [x] **Step 4: บันทึกทุกข้อผิดพลาดของแผนนี้ที่เจอตอนลงมือ**
...
> **ผลจริง 2026-08-17 — รวมทุกจุดที่แผนนี้ไม่ตรงกับของจริง**
>
> เขียนไว้ที่นี่ที่เดียวเพื่ออ่านรวดเดียว · รายละเอียดของแต่ละข้ออยู่ในบล็อก "ผลจริง" ใต้ task ของมันเอง
>
> | #   | ที่              | แผนเขียนว่า                                          | ของจริง                                                                                                                                                                                                                                                                                  |
> | --- | ---------------- | ---------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
> | 1   | ทุก task         | `pnpm --filter @pos/api test -- <path>`              | `--` ถูกส่งต่อเป็น argument ธรรมดา vitest จึงรันทั้งชุด (273 วินาที) · ต้องใช้ `pnpm --filter @pos/api exec vitest run <path>`                                                                                                                                                           |
> | 2   | Task 4           | `app.jwt.sign(payload, { jwtid })`                   | `@fastify/jwt` v9 เซ็นด้วย **fast-jwt** ซึ่งใช้ชื่อ `jti` · token เซ็นผ่านเงียบ ๆ โดยไม่มี `jti` แล้ว `/auth/me` 401 ทุกครั้งหลังล็อกอินสำเร็จ · เจอเพิ่มอีกสอง: option ระดับ call **แทนที่** option ระดับ plugin ไม่ได้ merge จึงต้องใส่ `expiresIn` ซ้ำ และ `expiresIn` เป็น**วินาที** |
> | 3   | Task 5           | `expect(purged).toBe(1)`                             | การล้างเป็น global โดยออกแบบ และไฟล์เทสต์อื่นทิ้งแถวไว้ในฐานข้อมูลที่ใช้ร่วมกัน · ต้อง assert `toBeGreaterThanOrEqual(1)` คู่กับ count เฉพาะ staffId                                                                                                                                     |
> | 4   | Task 6           | ใช้ `TextEncoder` ใน `@pos/shared` ได้               | มีตอนรันทั้ง Node และเบราว์เซอร์ แต่ `lib: ["ES2022"]` ล้วนไม่มีตัวประกาศชนิด · เติม DOM lib จะเปิด DOM API ทั้งชุดให้แพ็กเกจที่ห้ามแตะ DOM · เขียนตัวนับไบต์เอง                                                                                                                         |
> | 5   | Task 6           | เทสต์ล็อกบัญชีตามปกติ                                | bcrypt cost 12 × 11 ครั้ง ≈ 12 วินาที ชน timeout 5 วินาทีของ vitest · ให้ timeout เฉพาะเทสต์นั้น 60 วินาที ไม่ลดรอบและไม่ลด cost เพราะความช้าคือสิ่งที่กำลังทดสอบ                                                                                                                        |
> | 6   | Task 7           | `login(payload: unknown)`                            | TS เลือก overload แบบ chainable ของ `app.inject` ผลลัพธ์เลยไม่มี `statusCode`/`json()` · เทสต์รันผ่านแต่ typecheck แดง 13 บรรทัด · ใช้ `Record<string, unknown>`                                                                                                                         |
> | 7   | Task 6 เป็นต้นไป | จำนวนเทสต์คาดหวังของทุก task                         | เกินจริง 1 ตัวตั้งแต่ Task 6 (task นั้นมี 10 ไม่ใช่ 11) และผิดต่อกันไปทุก task หลังจากนั้น                                                                                                                                                                                               |
> | 8   | Task 9           | คอมเมนต์ "THIS FILE IS GITIGNORED" ใน `.env.example` | ไม่จริง — `.env.example` ถูก commit · ที่ gitignore คือ `.env`                                                                                                                                                                                                                           |
> | 9   | Task 9           | `prisma migrate reset --force`                       | เครื่องมือบังคับขอ consent จากผู้ใช้ก่อนคำสั่งที่ลบข้อมูล · พิสูจน์ทั้งสองเส้นทางจากฐานข้อมูล dev เดิมแทน แล้วเก็บกวาดแถวที่ค้าง                                                                                                                                                         |
> | 10  | Task 10          | `conflict('ข้อความ')`                                | `conflict(code, message)` รับสองอาร์กิวเมนต์ เพราะหน้าเว็บอ่าน `error` เป็นรหัสคงที่ ไม่ได้อ่านข้อความไทย                                                                                                                                                                                |
> | 11  | Task 10          | คนชั่วคราวชื่อขึ้นต้น `ทดสอบ`                        | `afterEach` เดิมกวาดทุกแถวที่ขึ้นต้นด้วย `PREFIX` ทิ้ง **ระหว่างเทสต์** · คนของ task นี้ต้องรอดข้าม inject หลายครั้งเพื่อล็อกอิน                                                                                                                                                         |
> | 12  | Task 11          | `SessionState` มีผู้ใช้แค่สองแอป                     | `business-day.ts` กับ `route-guards.tsx` ใน web-kit เองก็อ้างถึง · พอเป็น generic แล้ว build แดง `TS2314` ทันที · แก้ให้ทั้งคู่รับ `<C>` (`SessionState<unknown>` ใช้ไม่ได้เพราะ parameter เป็น contravariant)                                                                           |
> | 13  | Task 11          | ไม่ได้พูดถึงเทสต์หน้าล็อกอินหน้าร้าน                 | `LoginPage.test.tsx` assert วิธีเรียกแบบสามอาร์กิวเมนต์ ซึ่งคือสิ่งที่ task นี้เปลี่ยนพอดี · แก้เป็น object เดียว ค่าเหมือนเดิมทุกตัว                                                                                                                                                    |
> | 14  | Task 12          | `vi.importActual<typeof import('react-router-dom')>` | ผิดกฎ `@typescript-eslint/consistent-type-imports` ของโปรเจกต์ · ใช้ `import type * as ReactRouter` แทน · และ type import สองตัวที่เหลือค้างหลังลบ `staffList` ต้องลบตามด้วย                                                                                                             |
>
> **สิ่งที่แผนนี้ทำได้ดีกว่าแผนที่ 1:** ทุกข้อข้างบนเป็นรายละเอียดของเครื่องมือหรือของโค้ดรอบข้าง ไม่มีข้อไหนที่**การออกแบบ**ผิด · ลำดับ 13 task ไม่ต้องสลับ ไม่มี task ไหนต้องรื้อ

- [x] **Step 5: ตรวจทั้งหมดแล้ว commit**
...
- [x] `pnpm test` เขียวทั้ง 6 แพ็กเกจ
- [x] `pnpm typecheck` · `npx eslint .` · `npx prettier --check .` สะอาด
- [x] ล็อกอินหลังร้านด้วยอีเมล+รหัสผ่านได้จริงบนเบราว์เซอร์ และเซสชันหน้าร้านไม่ถูกเตะ
- [x] `curl -H "Host: office.example.com" http://localhost:3001/api/auth/staff` ได้ 404 เมื่อตั้ง `TILL_HOSTS`
- [x] กด logout แล้วเอาคุกกี้เดิมยิงซ้ำ ได้ 401
- [x] `grep -rn "ยังใช้ได้จนหมดอายุ" README.md` ไม่เจออะไร
- [x] ไม่มีรหัสผ่านจริงอยู่ในไฟล์ใดที่ถูก commit — ตรวจด้วย `git log -p | grep -i "OWNER_PASSWORD="`

> **ตรวจแล้วทั้งเจ็ดข้อ 2026-08-17**
>
> - `pnpm test` เขียวทั้ง 6 แพ็กเกจ **1,218 ตัว** · `typecheck` `eslint` `prettier --check` สะอาดทั้งหมด
> - ล็อกอินหลังร้านจริงบนเบราว์เซอร์ผ่าน · เซสชันหน้าร้านไม่ถูกเตะ (ดูบันทึกใต้ Task 12)
> - `curl -H "Host: office.example.com" .../auth/staff` → **404** พร้อม body `NOT_FOUND` · `Host: shop.example.com` → **200** (รัน API ชั่วคราวที่พอร์ต 3999 พร้อม `TILL_HOSTS=shop.example.com`)
> - **logout แล้วยิงคุกกี้ใบเดิมซ้ำด้วยมือ → 401** `เซสชันหมดอายุ กรุณาเข้าสู่ระบบใหม่` — ไม่ได้อาศัยว่าคุกกี้ถูกลบ แต่ส่งค่าเดิมกลับไปเอง · นี่คือคำสัญญาหลักของแผนนี้
> - `grep -rn "ยังใช้ได้จนหมดอายุ" README.md` ไม่เจอ (exit 1)
> - `git log -p --all | grep -i "OWNER_PASSWORD="` เจอแต่บรรทัดตัวอย่างที่ comment ไว้และค่าว่าง ไม่มีรหัสผ่านจริง
