...
Expected: **`/auth/branches /auth/login /auth/logout /auth/me /auth/staff` เท่านั้น** — ห้าตัวนี้คือกอง B ที่ตั้งใจให้ซ้ำ ถ้ามีตัวอื่นโผล่มาแปลว่าแยกผิด (ตอนร่างแผนเขียนไว้ว่า "ไม่มี" ซึ่งลืมนับกอง B ของตัวเอง)

นับเมธอดยืนยันอีกชั้น: `api-client` 48 (กอง C 43 + กอง B 5) · `api-office` 58 (กอง A 53 + กอง B 5) · รวมไม่ซ้ำ 101 เท่าเดิม
